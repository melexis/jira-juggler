...
class TestJiraJuggler(unittest.TestCase):
    ...
    def test_single_task_happy(self, jira_mock):
        ...
        data = self.JIRA_JSON_ISSUE_TEMPLATE.format(key=self.KEY1,
                                                    summary=self.SUMMARY1,
                                                    assignee=self.ASSIGNEE1,
                                                    estimate=self.ESTIMATE1,
                                                    depends=self.DEPENDS1)
        issue1 = json.loads(data, object_hook=lambda d: namedtuple('X', d.keys())(*d.values()))
        jira_mock_object.search_issues.side_effect = [[issue1], []]
    ...
    def test_task_depends(self, jira_mock):
        ...
        data = self.JIRA_JSON_ISSUE_TEMPLATE.format(key=self.KEY1,
                                                    ...
                                                    depends=self.DEPENDS1)
        issue1 = json.loads(data, object_hook=lambda d: namedtuple('X', d.keys())(*d.values()))
        data = self.JIRA_JSON_ISSUE_TEMPLATE.format(key=self.KEY2,
                                                    summary=self.SUMMARY2,
                                                    assignee=self.ASSIGNEE2,
                                                    estimate=self.ESTIMATE2,
                                                    depends=self.DEPENDS2)
        issue2 = json.loads(data, object_hook=lambda d: namedtuple('X', d.keys())(*d.values()))
        jira_mock_object.search_issues.side_effect = [[issue1, issue2], []]
        issues = juggler.juggle()
        jira_mock_object.search_issues.assert_has_calls([call(self.QUERY, maxResults=dut.JIRA_PAGE_SIZE, startAt=0),
